...
7. 가장 먼저, 좌/우 게시글들이 실제로 같은 구체적인 사건·정책·발언 등
   동일한 쟁점을 다루고 있는지 판단해라. "같은 인물이나 같은 시기의
   정치 상황"이라는 것만으로는 부족하다 — 예를 들어 한쪽은 어떤 정치인의
   개혁 성과를 다루고 다른 쪽은 그와 무관한 다른 사건(계엄령 가능성 등)을
   다룬다면, 같은 인물이 언급됐어도 서로 다른 쟁점이니 억지로 엮지 마라.
   서로 무관한 내용이거나(예: 한쪽은 애니메이션 감상평, 다른 쪽은 특정
   정당 지지 이야기처럼 전혀 다른 주제), 명확히 대비되는 구체적 쟁점을
   찾을 수 없다면 억지로 답을 지어내지 마라. 같은 사안을 다루더라도
   양쪽이 실질적으로 같은 의견/관측을 말하고 있을 뿐 서로 대비되는
   입장 차이가 없다면(예: 둘 다 "금리 인상이 환율을 안정시킬 것"이라는
   같은 관측만 하는 경우) 이것도 명확한 쟁점이 없는 것으로 본다 — 이런
   경우 no_clear_issue를 true로 하고 title/left_stance/right_stance는
   빈 문자열("")로 반환해라.
